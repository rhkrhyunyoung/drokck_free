...
def generate_launch_description():
    ...
    # --- 정확한 파일 경로 설정 ---
    imu_params_path = os.path.join(home_path, 'drokck_free/src/imu/gv7_params.yml')
    ...
    imu_node = Node(
        ...
        condition=IfCondition(PythonExpression(["'", season, "' in ['spring', 'autumn', 'winter']"]))
    )
    ...
    yolo_q = Node(
        ...
        executable='q',
        condition=IfCondition(PythonExpression(["'", season, "' in ['spring', 'summer']"]))
    )
    ...
    # 10. Linetracing (모든 계절 공통)
    linetracing = Node(
        package='linetracing_cpp', 
        executable='main2_node', 
        output='screen'
    )
